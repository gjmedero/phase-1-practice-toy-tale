...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(toyObjects)
    ...
    .then(toys => {
       ...
       let toysHTML = toys.map(function(toy) {
        ...
        <button data-id="${toy.id}" class="delete-like-btn">toy-chest</button>
      </div>
      `
       })
  ...
  toyFormContainer.addEventListener("submit", function(e) {
    ...
    .then(newToy => {
      ...
      <button data-id="${newToy.id}" class="delete-like-btn">toy-chest</button>
    </div>
      `
  ...
  toyCollection.addEventListener("click", (e) => {
    ...
    if (e.target.className === "delete-like-btn") {
      fetch(`http://localhost:3000/toys/${e.target.dataset.id}`, {
        method: "DELETE"
      })
      .then(res => {
